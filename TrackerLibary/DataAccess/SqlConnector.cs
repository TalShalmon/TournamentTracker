        ...
        private void SaveTournamentRounds(IDbConnection connection, TournamentModel model)
        {
            ...
            foreach (List<MatchupModel> round in model.Rounds)
            {
                foreach (MatchupModel matchup in round)
                {
                    var p = new DynamicParameters();
                    p.Add("@TournamentId", model.Id);
                    p.Add("@MathupRound", matchup.MatchupRound);
                    p.Add("@id", 0, dbType: DbType.Int32, direction: ParameterDirection.Output);
        ...
        public List<TournamentModel> GetTournament_All()
        {
            ...
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(GlobalConfig.CnnString(db)))
            {
                ...
                foreach (TournamentModel t in output)
                {
                    ...
                    // Populate Rounds   
                    List<MatchupModel> matchups = connection.Query<MatchupModel>("dbo.spMathups_GetByTournament", p, commandType: CommandType.StoredProcedure).ToList();
                    ...
                    List<MatchupModel> cuurRow = new List<MatchupModel>();
                    int currRound = 1;
                    ...
                    foreach (MatchupModel m in matchups)
                    {
                        ...
                        {
                            t.Rounds.Add(cuurRow);
                            cuurRow = new List<MatchupModel>();
                            currRound += 1;
                        }
